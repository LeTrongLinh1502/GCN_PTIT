...
for i_case, kwargs in enumerate(test_cases):
    ...
    if test_flag:
        ...
        results.append({
            ...
            'num_layers': args.nbaseblocklayer,
            'self_loops': args.withloop,
            'batch_norm': args.withbn,
            'pairnorm': args.use_pairnorm,
            'drop_edge': args.sampling_percent,
...
df_results.to_csv(OUTPUT_PATH + 'summary_results.csv', index=False)
plt.figure(figsize=(10,6))
plt.plot(df_results['drop_edge'], df_results['test_acc'], 'o-')
plt.xlabel('Drop Edge (p)')
plt.ylabel('Test Accuracy')
plt.title('Hiệu suất mô hình theo p')
plt.grid(True)
plt.show()
